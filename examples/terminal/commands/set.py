...
def set_table():
    ...
    # print attack params first
     # print attack params first
    attack_parameters = CFState.state().active_target.active_attack.options.attack_parameters
...
def get_options() -> dict:
    ...
    if not CFState.state().active_target:
        ...
        options = CFState.state().active_target.active_attack.options.get_all()
    else:
        options = {}
    return options
...
for k, v in get_options().items():
    parser.add_argument(f"--{k}", type=str, default=str(v["default"]))
...
def do_set(self, args: argparse.Namespace) -> None:
    ...
    CFState.state().get_active_target().active_attack.options.update(new_params)
